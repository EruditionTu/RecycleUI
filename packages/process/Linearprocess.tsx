...
const LinearProcessIcon: React.FC<{ status: string; percent: number }> =
...
const LinearProcess: React.FC<LinearProcessProps> = React.memo(
  (props: LinearProcessProps): React.ReactElement => {
    const { percent, strokeWidth, strokeColor, trailColor, showInfo, status } =
      props;
    const strokeStyle = {
    ...
    const processStatus =
      status === 'success' || status === 'exception' || status === 'normal'
        ? status
        : 'normal';
      ...
      <div className={classNames('linear-process', 'process-wapper')}>
        <div className="process-outer">
          <div className="process-inner" style={trailStyle}>
            <div
              className={classNames('process-done', processStatus)}
              style={strokeStyle}
            />
          </div>
        </div>
        {processShowInfo && (
          <LinearProcessIcon status={processStatus} percent={percent} />
        )}
      </div>
    );
  },
