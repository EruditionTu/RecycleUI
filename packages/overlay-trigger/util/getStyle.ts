import getScrollLength from '@/packages/common/util/dom/getScrollLength';
import getBoundingClientRect from '@/packages/common/util/dom/getBoundingClientRect';
import type { IBoundingClientRect } from '@/packages/common/util/dom/getBoundingClientRect';
import getOuterSize from './getOuterSize';
import type { Placement, OverlayStyle } from '../type';
...
export default function getStyle(options: GetStyleOptions) {
  ...
  const scrollTop = getScrollLength(
    (triggerDom as HTMLElement).ownerDocument!.documentElement,
    true,
  );
  const scrollLeft = getScrollLength((triggerDom as HTMLElement).ownerDocument!.documentElement);
  const trigger = {
    ...getBoundingClientRect(triggerDom as HTMLElement),
    ...getOuterSize(triggerDom as HTMLElement),
  };
  const popup = {
    ...getBoundingClientRect(popupDom as HTMLElement),
    ...getOuterSize(popupDom as HTMLElement),
  };
  ...
  switch (sty.placement) {
    ...
    case 'bottom':
    case 'top':
      sty.left -= (popup.width - trigger.width) / 2;
      break;
    case 'bottomRight':
    case 'topRight':
    ...
    case 'right':
    case 'left':
      sty.top -= (popup.height - trigger.height) / 2;
      break;
    case 'rightBottom':
    case 'leftBottom':
  ...
  if (autoAdjustOverflow) {
    ...
    if (placement && /^(left|right)/.test(placement)) {
      if (/(Top)$/.test(placement) && bottom + trigger.height < popup.height) {
        sty.top -= popup.height - bottom - trigger.height;
      }
      if (/(right|left)$/.test(placement) && bottom + trigger.height / 2 < popup.height / 2) {
        sty.top -= popup.height / 2 - bottom - trigger.height / 2;
      }
