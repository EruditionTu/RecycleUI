...
const CarouselItem: React.FC<CarouselSlidePropsType> = (
  props: CarouselSlidePropsType
): React.ReactElement => {
  const childrenRef = React.useRef(null);

  //鼠标点击开始事件
  const onItemMouseDown: (event: any) => void = React.useCallback(
    (event: any): void => {
  ...
  //移动端手指触摸事件
  const onItemTouchStart: (event: any) => void = React.useCallback(
    (event): void => {
