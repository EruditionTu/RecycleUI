...
const Tag:React.FC<TagPropsType&TagEventsType>=(props:TagPropsType&TagEventsType):React.ReactElement=>{
    ...
    React.useEffect(():void=>{
        if(props.color){
            if(props.effect==="dark"){
                ...
                if(closeRef.current)
                    closeRef.current.style.stroke="white";
            } else if(!props.effect||props.effect==="plain"){
                tagRef.current.style.color=props.color;
                if(closeRef.current)
                    closeRef.current.style.stroke=props.color;
            }
            tagRef.current.style.borderColor=props.color;
        }
    },[props.effect,props.color,tagRef,closeRef])
    const closeClick = React.useCallback((e:any):void=>{
        e.stopPropagation()//阻止React合成事件的冒泡
        setVisible(true)
        props.onClose&&props.onClose(e)
    },[setVisible])
    //关闭Icon渲染函数
    const renderCloseIcon:()=>React.ReactElement =React.useCallback(():React.ReactElement=>{
        if(props.closeIcon){
            return (
        ...
        return (
            <svg
                ...
                className={classnames(
                    ...
                    })} xmlns="https://www.w3.org/2000/svg" viewBox="0 0 52 52">
