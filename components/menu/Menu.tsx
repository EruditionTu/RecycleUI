...
import { ResetRecoilState, useRecoilState, RecoilRoot } from "recoil";
...
const Menu: React.FC<MenuType> = (props: MenuType): React.ReactElement => {
  ...
  const [activedSub, setActivedSub] = React.useState<Set<string>>(
    new Set<string>()
  ); //被展开的Sub
  const [selectedSub, setSelectedSub] = React.useState<Set<string>>(
    new Set<string>()
  ); //被选择的Sub
  const [selectedSubPath, setSelectedSubPath] = React.useState<Set<string>>(
    new Set<string>()
  ); //被选择的SubPath

  const [menuMode, setMenuMode] = useRecoilState(MenuModeState);
  const [subMenuCloseDelay, setSubMenuCloseDelay] = useRecoilState(
    MenuSubMenuCloseDelay
  );
  const [subMenuOpenDelay, setSubMenuOpenDelay] =
    useRecoilState(MenuSubMenuOpenDelay);
  ...
  const [menuSelectable, setMenuSelectable] =
    useRecoilState(MenuSelectableState);
  const [menuTriggerSubMenuAction, setMenuTriggerSubMenuAction] =
    useRecoilState(MenuTriggerSubMenuActionState);
  const [menuInlineIndent, setMenuInlineIndent] = useRecoilState(
    MenuInlineIndentState
  );
  ...
  React.useEffect((): void => {
    ...
    setSelectedItems(
      Array.isArray(props.defaultSelectedKeys)
        ? new Set<string>(
            props.defaultSelectedKeys.filter((x) => typeof x === "string")
          )
        : new Set<string>()
    );
  }, []);
  ...
  React.useEffect((): void => {
    if (Array.isArray(props.openKeys)) return;
    setActivedSub(
      Array.isArray(props.defaultOpenKeys)
        ? new Set<string>(
            props.defaultOpenKeys.filter((x) => typeof x === "string")
          )
        : new Set<string>()
    );
  }, []);
  ...
  React.useEffect((): void => {
    if (!Array.isArray(props.selectedKeys)) return;
    setSelectedItems(
      new Set<string>(props.selectedKeys.filter((x) => typeof x === "string"))
    );
  }, [props.selectedKeys]);
  ...
  React.useEffect((): void => {
    if (!Array.isArray(props.openKeys)) return;
    setActivedSub(
      new Set<string>(props.openKeys.filter((x) => typeof x === "string"))
    );
  }, [props.openKeys]);
  ...
  //选择Item
  const selectItem: (keyName: string) => void = React.useCallback(
    (keyName: string): void => {
      if (props.multiple) {
  ...
  //取消选择-仅多选可以使用
  const cancelSelectItem: (keyName: string) => void = React.useCallback(
    (keyName: string): void => {
      if (props.multiple) {
  ...
  //打开Sub
  const open: (keyName: string, multiple: boolean) => void = React.useCallback(
    (keyName: string, multiple: boolean): void => {
      if (multiple) {
  ...
  //关闭某个key的Sub
  const cancelOpen: (keyName: string) => void = React.useCallback(
    (keyName: string): void => {
      setActivedSub((oldActivedSub: Set<string>): Set<string> => {
  ...
  //选择某个key的Sub
  const selectSubPath: (keyName: string) => void = React.useCallback(
    (keyName: string): void => {
      if (props.multiple) {
        //可以多选
  ...
  //不再选择某个key的Sub
  const cancelSelectSubPath: (keyName: string) => void = React.useCallback(
    (keyName: string): void => {
      setSelectedSubPath((oldSelectedSubPath): Set<string> => {
  ...
  return (
    <MenuContext.Provider
      value={{
        ...
        onDeselect:
          typeof props.onDeselect === "function"
            ? props.onDeselect
            : (): void => {},
        onOpenChange:
...
const MenuWarpper: React.FC<MenuType> = (
  props: MenuType
): React.ReactElement => {
  return (
    <RecoilRoot>
