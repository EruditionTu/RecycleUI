...
import { MenuMultipleState } from "./state/MenuAtoms";
import MenuContext from "./MenuContext";
import { MenuItemPropTypes } from "./types/MenuItemTypes";
import { MenuSelectableState, MenuThemeState } from "./state/MenuAtoms";
...
const MenuItem: React.FC<MenuItemPropTypes> = (
  ...
  return (
    <li
      ...
      className={classnames("menu-item", `menu-item-${theme}`, {
        [`menu-item-${theme}-active`]: actived,
        [`menu-item-${theme}-selected`]: selected,
      })}
    >
      <span className={"menu-item-content"}>{props.label || "MenuItem"}</span>
    </li>
  );
};
