...
type ItemType = (typeof MenuItem|typeof SubMenu|typeof MenuItemGroup|typeof MenuDivider );

let compileToReactComponent:(objectArray:Array<ItemObject>)=>React.ReactNode[];
compileToReactComponent=(objectArray:Array<ItemObject>):React.ReactNode[]=>{
    ...
    if(objectArray&&Array.isArray(objectArray)){
        objectArray.forEach((item:ItemObject,index:number):void=>{
            ...
            let keyname:string = (item.key&&typeof item.key === "string")? item.key: randomKey(100);
