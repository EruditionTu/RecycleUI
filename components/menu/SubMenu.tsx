...
import { compileToReactComponent } from "./tools/compileToReactComponents";
import { ItemObject } from "./types/MenuTypes";
import { SubMenuPropTypes, PopupTypes } from "./types/SubMenuTypes";
...
const Popup: React.FC<PopupTypes> = (props: PopupTypes): React.ReactElement => {
  ...
  //选择Item
  const select: (keyName: string, path: Array<string>, e: any) => void =
    React.useCallback(
      (keyName: string, path: Array<string>, e: any): void => {
      ...
      [selectable, props.path, context.selectSubPath, context.selectItem]
    );
  //取消选择-仅多选可以使用
  const cancelSelect: (keyName: string, path: Array<string>, e: any) => void =
    React.useCallback(
      (keyName: string, path: Array<string>, e: any): void => {
        if (!selectable) return;
        context.onDeselect({
  ...
  //点击展开子菜单
  const ClickOpenSub: (keyName: string) => void = React.useCallback(
    (keyName: string): void => {
      if (!openable) return;
      if (mode === "inline") {
  ...
  //鼠标进入展开子菜单
  const MouseEnter: (keyName: string) => void = React.useCallback(
    (keyName: string): void => {
      if (!openable) return;
      if (mode === "inline") return;
      if (
        triggerSubMenuAction === "hover" ||
        (triggerSubMenuAction === "click" && props.actived)
      ) {
  ...
  //鼠标出去触发的事件
  const MouseLeave: (keyName: string) => void = React.useCallback(
    (keyName: string): void => {
      if (!openable) return;
      if (mode === "inline") return;
    ...
    [context.cancleOpen, subMenuCloseDelay, timer.current, mode, openable]
  );
  return (
    <ul
      className={classnames("sub-menu-popup", `sub-menu-popup-${theme}`, {
                ...
                className={classnames(
                  ...
                  `sub-menu-popup-item-${theme}`,
                  {
                    [`sub-menu-popup-item-${theme}-selected`]:
                      (child.type === "MenuItem" &&
                        context.selectedItems.has(child.key)) ||
                      (child.type === "SubMenu" &&
                        context.selectedSub.has(child.key)),
                    [`sub-menu-popup-item-${theme}-selected-nopopup`]:
                      child.type === "MenuItem" &&
                      context.selectedItems.has(child.key),
                  }
                )}
                onMouseEnter={() => {
                  if (child.type === "SubMenu") MouseEnter(child.key);
                }}
                onMouseLeave={() => {
                  if (child.type === "SubMenu") MouseLeave(child.key);
                }}
              >
                <div
                ...
                >
                  <span
                    style={{
                      marginLeft: `${
                        mode === "inline" ? (props.depth + 1) * inlineIndent : 0
                      }px`,
                    }}
                  ...
                  {child.type === "SubMenu" && mode === "inline" && (
                    <i
                      className={classnames("sub-menu-popup-item-arrow", {
...
const SubMenu: React.FC<SubMenuPropTypes> = (
  props: SubMenuPropTypes
): React.ReactElement => {
  const context = React.useContext(MenuContext);
  ...
  }, [context.activedSub, props.name, setActived]);
  React.useEffect((): void => {
    setSelected((): boolean => {
      let status: boolean = context.selectedSub.has(props.name);
      return status;
    });
  }, [context.selectedSub, setSelected, props.name]);

  //鼠标点击事件
  const MouseClick: (e: any) => void = React.useCallback(
    (e: any) => {
  ...
  return (
    <li
      className={classnames("sub-menu", `sub-menu-${theme}`, {
    ...
    >
      <div
        className={classnames({ "sub-menu-titleContainer": mode === "inline" })}
        onClick={(e: any) => MouseClick(e)}
      >
        <span className={classnames("sub-menu-title")}>{label}</span>
        {/*横版的选择条*/}
        {theme === "light" && mode === "horizontal" ? (
          <div
            className={classnames("after", {
        ...
        {mode === "inline" && (
          <i
            className={classnames("sub-menu-arrow", {
              "arrow-top": !actived,
